...
with tab2:
    ...
    st.write(
        f"실측 설문 조사 데이터 분석 결과, 조작변인 기여율인 개인(299) : 사회(257) : 또래+미디어(302) 비율을 시스템 가치로 대입했을 때, "
        f"현 상태 유지 시 청소년 집단의 최대 중독 임계 수치(K)는 {K_raw:.1f}% 수준에 달하는 것으로 예측되었습니다. "
        f"그러나 고안된 3대 정책(스트레스 완화 {policy_personal}%, 미디어 통제 {policy_peer_media}%, 인프라 강화 {policy_social}%)을 동시 가동할 경우, "
