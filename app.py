...
# 2. 사이드바 디자인 및 데이터 입력 (3대 조작변인 및 세분화 요인 위험 요소 설정)
...
media = st.sidebar.slider("디지털 미디어 및 SNS 노출 빈도", 1.0, 10.0, 7.0, step=0.1)
peer = st.sidebar.slider("또래 집단 압박 및 소외감", 1.0, 10.0, 5.0, step=0.1)

# 사회적 요인 세분화 (총 가중치 257을 세분화하여 배분)
st.sidebar.markdown("### 🟢 3. 사회적 요인 (총 가중치: 257)")
# 3-A. 매크로 사회 구조 및 경제적 요인 (가중치 130)
macro_socioeconomic = st.sidebar.slider("매크로 사회 구조 및 경제적 불안정성", 1.0, 10.0, 6.0, step=0.1, 
                                        help="지역사회 인프라 편차, 제도적 예방망 사각지대, 경제적 양극화 수준")

# 3-B. 마이크로 시스템 및 관계적 사회 요인 (가중치 127)
micro_interpersonal = st.sidebar.slider("마이크로 시스템 및 관계적 결핍도", 1.0, 10.0, 5.0, step=0.1,
                                        help="가정 내 지지 기반 약화, 1차적 관계 지지망 방임도")

# [수학적 기초 모델 설계 (세분화 가중치 비율 적용 - 299 : 302 : 130 : 127)]
total_weight = 299 + 302 + 130 + 127  # 988
w_personal = 299 / total_weight
w_peer_media = 302 / total_weight
w_macro = 130 / total_weight
w_micro = 127 / total_weight

# [현상 유지 상태의 위험 지수 및 로지스틱 변수 계산]
risk_index_raw = (
    (w_personal * stress) + 
    (w_peer_media * ((media + peer) / 2.0)) + 
...
with tab1:
    ...
    with col_raw1:
        ...
        st.caption("※ 모든 요인이 위험 요인으로 작동하므로, 각 지표의 누적치가 환경 확산 계수(a)와 최종 수렴 한계선인 임계선(K)을 동시에 밀어 올립니다.")
    ...
    st.write(
        f"개인적 스트레스 요인({w_personal*100:.1f}%), 관계/미디어 요인({w_peer_media*100:.1f}%), "
        f"그리고 거시 사회구조적 요인({w_macro*100:.1f}%) 및 미시 관계적 요인({w_micro*100:.1f}%)이 "
        f"모두 복합 위험 요소로 작동하고 있습니다. 이에 따라 통합 중독 취약성 지수가 상승하여, "
        f"초기 확산 속도를 지배하는 환경 확산 계수(a)가 가파르게 형성되며, 최종 수렴치인 최종 누적 잠재 중독율 임계선(K) 역시 **{K_raw:.1f}%**라는 높은 수준으로 고착화됩니다."
    )

# ---------------------------------------------------------------- Project Tab 2
with tab2:
    st.subheader("🛠️ 요인별 구체적 정책 및 개입")
    st.write("각 요인의 위험도를 낮추기 위해 기획된 구체화된 정책 카드를 조절하여 변화 폭을 시뮬레이션하세요.")
    
    # 세분화된 4가지 요인 대안에 맞춘 대칭 레이아웃 배치
    policy_col1, policy_col2, policy_col3, policy_col4 = st.columns(4)
    
    with policy_col1:
        st.markdown("<div style='background-color:#eff6ff; padding:15px; border-radius:8px; border:1px solid #bfdbfe; height:240px;'>", unsafe_allow_html=True)
    ...
    with policy_col2:
        st.markdown("<div style='background-color:#fff7ed; padding:15px; border-radius:8px; border:1px solid #fed7aa; height:240px;'>", unsafe_allow_html=True)
        st.markdown("### 📱 2. 관계+미디어 대안")
        st.caption("**[디지털 디톡스 및 자치 규약]**")
        policy_peer_media = st.slider("또래 동조 및 노출 통제율 (%)", 0, 50, 25, step=5)
        st.markdown("</div>", unsafe_allow_html=True)
        
    with policy_col3:
        st.markdown("<div style='background-color:#f0fdf4; padding:15px; border-radius:8px; border:1px solid #bbf7d0; height:240px;'>", unsafe_allow_html=True)
        st.markdown("### 🏛️ 3-A. 매크로 사회 구조 대안")
        st.caption("**[예방 인프라 확충 및 제도 마련]**")
        policy_macro = st.slider("사회구조 위험 요인 개선율 (%)", 0, 100, 40, step=10)
        st.markdown("</div>", unsafe_allow_html=True)

    with policy_col4:
        st.markdown("<div style='background-color:#faf5ff; padding:15px; border-radius:8px; border:1px solid #e9d5ff; height:240px;'>", unsafe_allow_html=True)
        st.markdown("### 🏡 3-B. 마이크로 관계 대안")
        st.caption("**[가족 치유 및 밀착 상담 케어]**")
        policy_micro = st.slider("관계 및 돌봄 체계 개선율 (%)", 0, 100, 40, step=10)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    # 위험 요인들의 결합치 계산
    risk_index_opt = (
        (w_personal * stress_opt) + 
        (w_peer_media * ((media_opt + peer_opt) / 2.0)) + 
        (w_macro * macro_opt) + 
        (w_micro * micro_opt)
    )
    ...
    st.write(
        f"분석 결과, 기획된 **다차원 복합 정책(스트레스 완화 {policy_personal}%, 미디어 통제 {policy_peer_media}%, "
        f"매크로 인프라 구축 {policy_macro}%, 마이크로 관계 돌봄 강화 {policy_micro}%)**을 동시에 작동시킬 때, "
        f"청소년 사회의 잠재적 중독 한계선을 **{K_raw:.1f}%**에서 **{K_opt:.1f}%**로 크게 낮출 수 있음이 수학적 모델링으로 증명되었습니다. "
        "단일 차원의 접근이 아닌, 거시적 제도 보완과 미시적 유대 복원이 상호 보완적으로 작용할 때 정책적 효율성이 극대화됨을 시사합니다."
    )
