...
# 2. 사이드바 - 설문조사 기반 환경 변수 입력 (3대 조작변인 위험 요소 설정)
st.sidebar.header("📋 기초 설문조사 데이터 입력")
st.sidebar.write("실제 설문조사에서 도출된 영역별 위험 지수(1~10점)를 입력하세요.")
...
with tab1:
    ...
    with col_raw1:
        ...
        st.text(f"- 현재 환경 확산 계수 (a): {a_raw:.5f}")
        st.text(f"- 현재 환경 제어 계수 (b): {b_raw:.5f}")
        st.write(
            "※ 모든 요인이 위험 요인으로 작동하므로, 세 가지 지표의 누적치가 환경 확산 계수(a)와 "
            "최종 수렴 한계선인 임계선(K)을 동시에 밀어 올리는 구조를 가집니다."
        )
...
with tab2:
    ...
    with policy_col3:
        ...
        st.caption("**[Wee클래스 인프라 확충 및 적극적 예방 교육]**")
        policy_social = st.slider("사회적 위험 요인 개선율 (%)", 0, 100, 40, step=10)

    # 개입 시나리오가 반영된 새로운 수학적 변수 계산 (각 위험 수치 감소 적용)
    ...
    with opt_col1:
        st.success(f"❇️ **개선된 최종 누적 잠재 중독율 임계선 (K_opt):** **{K_opt:.1f}%**")
